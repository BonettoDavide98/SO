...
int main () {
	...
	int *msgqueue_nave = malloc(numporti * sizeof(int));
	...
	ports_positions[4].x = max_x;
	ports_positions[4].y = max_y;
	...
	for(i = 0; i < numporti; i++) {
		...
		if((msgqueue_porto[i] = msgget(IPC_PRIVATE, IPC_CREAT | 0600)) == -1) {
		...
		for(int j = 0; j < nummerci; j++) {
			if(rand()%2 == 1) {
				...
				printf("ADDED: %d OF %d TO PORT %d\n" , ports_shm_ptr_aval[i][j].qty, j, i);
			} else {
				ports_shm_ptr_req[i][j].qty = rand() % maxrandommerce;
				printf("ADDED REQUEST: %d OF %d TO PORT %d\n" , ports_shm_ptr_req[i][j].qty, j, i);
			}
		}
...
int getRequesting(char *posx_s, char *posy_s, struct position * portpositions, struct merce ** portscontents, int merce, int nporti) {
